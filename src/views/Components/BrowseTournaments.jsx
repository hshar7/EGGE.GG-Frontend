...
class BrowseTournaments extends React.Component {
    ...
    componentDidMount() {
        axios.get(`${base}/tournament/tournamentsByGameId/${this.props.match.params.gameId}`).then(response => {
    ...
    render() {
        ...
        this.state.tournaments.forEach(tour => {
            tournamentList.push(<GridItem xs={3} style={{ "background-color": "white", "cursor": "pointer" }} onClick={() => { this.redirect(tour.id) }}>
                <h5>{tour.name}</h5>
                <img
                    src={tour.game.url}
                    alt={tour.name}
                    className={classNames(classes.browseStyle, classes.imgRaised, classes.imgRounded, classes.imgFluid)}
                />
            </GridItem>)
        });
